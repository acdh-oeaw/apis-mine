...
from .provide_data import oebl_persons
...
class ProfessionAC(autocomplete.Select2QuerySetView):
    ...
    def get_queryset(self):
        if self.q:
            query = self.q
            match = ProfessionType.objects.filter(name__icontains=query)
            return match
        else:
            return []
...
class OeblPersons(autocomplete.Select2QuerySetView):
    ...
    def get_queryset(self):
        if self.q:
            ...
            label_match = list(
                [
                    x.temp_entity.get_child_entity()
                    for x in Label.objects.filter(temp_entity__in=oebl_persons).filter(
                        label__icontains=query
                    )
                ]
            )
            match = oebl_persons.filter(
                Q(name__icontains=query) | Q(first_name__icontains=query)
            )
