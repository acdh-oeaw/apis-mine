    ...
    def members(
        ...
        **kwargs,
        ...
        if start is None and end is None and institutions is None:
            return self.filter(
                personinstitution_set__relation_type_id__in=ids
            ).distinct()
        qs = self.annotate(
        ...
        if institutions is not None:
            if isinstance(institutions[0], str):
                institutions = list(
                    Institution.objects.filter(name__in=institutions).values(
                        "pk", flat=True
                    )
                )
...
class PAASPerson(Person):
    ...
    def get_place_of_birth(
        self, include_country: bool = False
    ) -> typing.Union[Place, typing.Tuple[Place, Place]]:
        ...
        if include_country:
            birth_country = birth_place.related_placeB.filter(
                relation_type_id__in=located_in
            )
    ...
    def nobelprizes(
        self,
    ) -> typing.Union[None, typing.List[typing.Tuple[datetime.date, str, Institution]]]:
        nobelpreise = getattr(id_mapping, "NOBEL_PREISE")
        preistr = getattr(id_mapping, "RELATION_PREISTRAEGER")
        nb = self.personinstitution_set.filter(
            related_institution_id__in=nobelpreise, relation_type_id__in=preistr
        )
        if nb.count() == 0:
            return None
        else:
            ...
            for nb1 in nb:
                res.append(
                    (
                        nb1.start_date,
                        nb1.related_institution.name,
                        nb1.related_institution,
                    )
                )
            return res
...
class PAASInstitution(Institution):
    ...
    def get_class(self):
        try:
            return self.related_institutionB.get(
                relation_type_id=2, related_institutionB_id__in=[1, 2, 3]
            )
        except:
            return None
    ...
    def get_members(
        ...
        members: typing.Union[
            typing.List[typing.Literal["president", "member"]], typing.List[int], None
        ] = None,
        start: typing.Optional[str] = None,
        end: typing.Optional[str] = None,
        **kwargs,
        ...
        if members is not None:
            if len(members) > 0:
                if isinstance(members[0], str):
                    ...
                    for mem in members:
                        members_new.extend(
                            getattr(id_mapping, "INSTITUTION_MEMBERS_TYPES")[mem]
                        )
    ...
    def get_members_persons(
        ...
        **kwargs,
        ...
        ids = list(
            set(
                self.get_members(members=members, start=start, end=end).values_list(
    ...
    def get_memberships_dict(
        ...
        **kwargs,
    ) -> typing.List[MitgliedschaftInstitutionDict]:
    ...
    def get_structure(
        ...
        relations: typing.Union[
            typing.List[typing.Literal["ist Teil von", "hat Untereinheit"]],
            typing.List[int],
            None,
        ] = None,
        start: typing.Optional[str] = None,
        ...
        **kwargs,
    ) -> typing.List[InstitutionInstitutionDict]:

        res = []
        q_dict = {}
        if relations is None:
            q_dict["relation_type_id__in"] = getattr(
                id_mapping, "INSTITUTION_STRUCTURE_IDS"
            )
        ...
        if start is not None:
            ...
            q_dict["start_date__lte"] = convert_date(end)
            q_dict["end_date__gte"] = convert_date(start)
        for instinst in self.related_institutionA.filter(**q_dict).exclude(
            related_institutionA_id__in=getattr(
                id_mapping, "GESAMTAKADEMIE_UND_KLASSEN"
            )
        ):
            res.append(
                {
                    ...
                    "relation_label": instinst.relation_type.label
                    if instinst.related_institutionA == self
                    else instinst.relation_type.label_reverse,
                    "institution": instinst.related_institutionB
                    if instinst.related_institutionA == self
                    else instinst.related_institutionA,
                    "institution_label": str(instinst.related_institutionB)
                    if instinst.related_institutionA == self
                    else str(instinst.related_institutionA),
        ...
        for instinst in self.related_institutionB.filter(**q_dict).exclude(
            related_institutionB_id__in=getattr(
                id_mapping, "GESAMTAKADEMIE_UND_KLASSEN"
            )
        ):
            res.append(
    ...
    def get_prize_recipients(
        ...
        **kwargs,
        ...
        return [
            {
                "preistraeger": x.related_person,
                "date": x.start_date,
                "abgelehnt": True
                if x.relation_type_id
                in getattr(id_mapping, "RELATION_PREISTRAEGER_ABGELEHNT")
                else False,
            }
            for x in self.personinstitution_set.filter(
                relation_type_id__in=getattr(id_mapping, "RELATION_PREISTRAEGER")
                + getattr(id_mapping, "RELATION_PREISTRAEGER_ABGELEHNT")
            ).order_by("start_date")
        ]
    ...
    def get_prize_stifter(
        ...
        **kwargs,
        ...
        return [
            {"stifter": x.related_person, "date": x.start_date}
            for x in self.personinstitution_set.filter(
                relation_type_id__in=getattr(id_mapping, "RELATION_PREIS_STIFTER")
            ).order_by("start_date")
        ]

    def _get_relation_label_history(
        self,
        relation,
        relations_query: typing.List[
            typing.Literal["Institutionelle Vorläufer", "Institutionelle Nachfolger"]
        ] = ["Institutionelle Vorläufer", "Institutionelle Nachfolger"],
    ):
        if relation.related_institutionA == self:  # normal direction
            if (
                relation.relation_type_id
                in getattr(id_mapping, "INSTITUTION_HISTORY_IDS")[
                    "Institutionelle Vorläufer"
                ]
                and "Institutionelle Vorläufer" in relations_query
            ):
                ...
                and "Institutionelle Nachfolger" in relations_query
            ):
    ...
    def get_history(
        ...
        **kwargs,
        ...
        q_dictA = {}
        q_dictB = {}
        q_dictA["relation_type_id__in"] = q_dictB["relation_type_id__in"] = list(
            chain.from_iterable(
                [
                    getattr(id_mapping, "INSTITUTION_HISTORY_IDS")[rel_ids]
                    for rel_ids in relations
                ]
            )
        )
        if len(relations) < 2:
            q_dictA["relation_type_id__in"] = list(
                chain.from_iterable(
                    [
                        getattr(id_mapping, "INSTITUTION_HISTORY_IDS")[rel_ids]
                        for rel_ids in relations
            ...
            q_dictB["relation_type_id__in"] = list(
                set(
                    chain.from_iterable(
                        [
                            x
                            for x in getattr(
                                id_mapping, "INSTITUTION_HISTORY_IDS"
                            ).values()
                        ]
                    )
                )
            ...
            res.append(
                {
                    ...
                    "relation_label": str(instinst.relation_type)
                    if instinst.related_institutionA == self
                    else str(instinst.relation_type.name_reverse),
                    "institution": instinst.related_institutionB
                    if instinst.related_institutionA == self
                    else instinst.related_institutionA,
                    "institution_label": str(instinst.related_institutionB)
                    ...
                    "start": instinst.start_date,
                    "end": instinst.end_date,
                    "start_date_related_institution": instinst.related_institutionB.start_date
                    if instinst.related_institutionA == self
                    else instinst.related_institutionA.start_date,
                    "end_date_related_institution": instinst.related_institutionB.end_date
                    if instinst.related_institutionA == self
                    else instinst.related_institutionA.end_date,
                }
            )
        ...
        if full_history:
            for instA in res:
                res_2 = PAASInstitution.objects.get(
                    pk=instA["institution"].pk
                ).get_history(relations=[instA["relation"]])
                for res_3 in res_2:
                    if res_3 not in res:
                        res.append(res_3)
        res = sorted(
    ...
    def get_website_data(self, res=None):
        ...
        for x in vor_nachf:
            if x["relation"] == "Institutionelle Vorläufer":
                ...
                lst_nach.append(x)
        if len(lst_vor) > 0:
            res["daten_institution"]["INSTITUTIONELLE VORLÄUFER"] = [
                f"{lst_vor[-1]['relation_label']} <a href='{lst_vor[-1]['institution_link']}'>{lst_vor[-1]['institution_label']}</a></br>{lst_vor[-1]['start_date_related_institution'].strftime('%Y') if lst_vor[-1]['start_date_related_institution'] else ''} - {lst_vor[-1]['end_date_related_institution'].strftime('%Y') if lst_vor[-1]['end_date_related_institution'] else ''}"
            ]
        if len(lst_nach) > 0:
            res["daten_institution"]["INSTITUTIONELLE NACHFOLGER"] = [
                f"{lst_nach[0]['relation_label']} <a href='{lst_nach[0]['institution_link']}'>{lst_nach[0]['institution_label']}</a></br>{lst_nach[0]['start_date_related_institution'].strftime('%Y') if lst_nach[0]['start_date_related_institution'] else ''} - {lst_nach[0]['end_date_related_institution'].strftime('%Y') if lst_nach[0]['end_date_related_institution'] else ''}"
            ]
        if len(lst_vor) > 1:
            res["daten_institution"]["INSTITUTIONELLE VORLÄUFER"] = [
                f"<a href='{ii['institution_link']}'>{ii['institution_label']}</a></br>{ii['start_date_related_institution'].strftime('%Y') if ii['start_date_related_institution'] else ''} - {ii['end_date_related_institution'].strftime('%Y') if ii['end_date_related_institution'] else ''}"
                for ii in lst_vor[:-1]
            ] + res["daten_institution"]["INSTITUTIONELLE VORLÄUFER"]
        if len(lst_nach) > 1:
            res["daten_institution"]["INSTITUTIONELLE NACHFOLGER"].extend(
                [
                    f"<a href='{ii['institution_link']}'>{ii['institution_label']}</a></br>{ii['start_date_related_institution'].strftime('%Y') if ii['start_date_related_institution'] else ''} - {ii['end_date_related_institution'].strftime('%Y') if ii['end_date_related_institution'] else ''}"
                    for ii in lst_nach[1:]
                ]
            )
        if self.kind_id in getattr(id_mapping, "INSTITUTION_TYPE_AKADEMIE"):
            for mem in self.get_memberships_dict():
                for k, v in getattr(id_mapping, "INSTITUTION_TYPE_MEMBERSHIP").items():
                    if mem["mitgliedschaft"].id in v:
                        ...
                        res["daten_institution"][k].append(
                            f"<a href='/person/{mem['person'].id}'>{mem['person']}<a/> {'(Stv.)' if 'stellvertreter' in str(mem['mitgliedschaft']).lower() else ''} {mem['start'].strftime('%Y') if mem['start'] else ''} - {mem['end'].strftime('%Y') if mem['end'] else ''}"
                        )
        res["struktur"] = []
        ...
        related_inst = list(
            self.related_institutionB.filter(
                related_institutionB_id__in=[1, 2, 3], relation_type_id__in=[2, 99]
            ).order_by("start_date")
        ) + list(
            self.related_institutionA.filter(
                related_institutionB_id__in=[1, 2, 3], relation_type_id__in=[2, 99]
            ).order_by("start_date")
        )
        if len(related_inst) == 1:
            ...
            for rel_inst_2 in related_inst:
                lst_untertitel.append(
                    f"{self.kind.name}{' der ' + str(rel_inst_2.related_institutionB) if rel_inst_2.related_institutionB else ''} {rel_inst_2.start_date.strftime('%Y') if rel_inst_2.start_date else ''} - {rel_inst_2.end_date.strftime('%Y') if rel_inst_2.end_date else ''}"
                )
